...
| **Analyst sidebar watchlist** ([`AnalystDetailClient.tsx`](../components/analysts/AnalystDetailClient.tsx) + [`WatchlistRow`](../components/ui/trade-row.tsx)) | `Thesis WHERE status='WATCHING'`. Each row renders ticker + price + the day's move ($ and %, same `GainPair` as a trade row) + a subline of "Awaiting review" (unresearched seed) or plain "Watching". Direction is not printed on the row — it only seeds the thesis sheet. | The watchlist UI. Remove action calls `removeWatchlistItem` → flips to ARCHIVED. |
