...
**A big winner is not trimmed.** `GAIN_FROM_ENTRY` takes an optional `skipIfPeakGainPct`: once the position's tracked peak has run that far off the buy, the rung is off for good (playbook Part F). A fill writes it onto the partial sale for a trade or a target position, at 20%, so an ordinary winner is still de-risked at 2R while a runner is held and managed on the trail. It reads `Position.peakPrice`, the same water mark the trail reads, so the two cannot drift apart. MU's peak gain is 15.5% and keeps its partial; a stock like SMMT, up 30.5% off the buy in 17 days, does not.
