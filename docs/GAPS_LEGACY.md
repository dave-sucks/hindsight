# Hindsight — Gaps (LEGACY)

> **⚠ This is the LEGACY tracker.** It was the 6-week thesis-architecture-rework punch list, ~80% shipped as of the first live promotion on 2026-05-26. The live tracker is now [`GAPS.md`](./GAPS.md), which carries forward only the items that block or degrade live trading.
>
> Items that didn't make it into the new file are deferred indefinitely — they weren't important enough to land before live, so they aren't the live loop's bottleneck. Reopen in `GAPS.md` only if production data shows one biting again.
>
> The carry-forward items moved to the new file (with renumbering): old P0-14 → new P0-1, old P0-15 → new P0-2, old P1-23 → new P1-3 (targetPrice), old P1-24 → new P1-4 (promotedAt), old P1-25 → new P1-5 (writer hallucination). Old P1-22 (place_trade staleness gate) was killed — replaced by the review-driven refresh design in new P1-1.
>
