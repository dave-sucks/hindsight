...
### ✅ P1-26 — Legacy briefing fully removed — **CLOSED 2026-06-26** ([#451](https://github.com/dave-sucks/hindsight/pull/451) code + [#459](https://github.com/dave-sucks/hindsight/pull/459) tables)
Two-stage removal of the dead `AnalystBriefing` + `MorningBrief` systems (replaced by `PortfolioDigest`):
- **#451** — most of the code: writer, brief UI, `/intelligence` Briefs tab, run-detail brief card, `getAnalystDetail` lists, types, workflow-registry entry. (Bug caught in-PR: an accidentally-committed `lib/generated` symlink broke `prisma generate` on Vercel — untracked + `.gitignore` broadened.)
- **#459** — re-verify caught **4 live refs #451 MISSED** (`read-run`'s `run.briefing`, the health-tab `morningBrief.count`, the `analystBriefing.deleteMany` cascade, and the **`read_database` string-literal allowlist** — `tsc` can't catch string model names). Removed those + the dead `recentRuns` query + the 2 Prisma models + relation fields + the `DROP TABLE` migration.
- **Prod DROP applied 2026-06-26 via MCP** (`drop_legacy_briefing_tables`): `AnalystBriefing` (641 rows) + `MorningBrief` (120 rows) dropped; verified gone, 0 incoming FKs, `PodcastSegmentBriefing`/`PortfolioDigest`/`AgentConfig`/`ResearchRun`/`Thesis` intact.
- **Lesson:** "code fully deleted" needs a re-grep for **string-literal model refs** (read_database / read-run allowlists), not just a green `tsc`.

### ✅ P1-29 — Reject-message instructions are write-only — **CLOSED 2026-06-26** ([#457](https://github.com/dave-sucks/hindsight/pull/457))
The principal's reject-dialog instructions ("raise my stop, ride past target") were stored as `ThesisUpdate(PROPOSAL_REJECTED)` but never read or acted on — the agent treated every reject as a bare "soft no" and re-proposed the same exit. #457 closed the loop (verified on main): `rejectProposal` sets `nextReviewAt=now` (`execute.ts:445`) → the next run's `get_theses` surfaces a `principalDirective` per thesis (`get-theses.ts`, `classifyPrincipalDirective`) → the daily-run prompt (`system-prompt.ts:189`, `:242`) tells the agent to **execute the instruction, not just note it**. Plus direct control: inline target/stop editing in the trigger popover + "Edit & approve" on proposals. **Caveat:** wired + verified in code, but **not yet runtime-proven** — the motivating case (CRDO) was SOLD at its $270 target on **06-23, two days before #457 merged (06-25)**, with the stop never raised from $198.91, so the bug bit once more before the fix landed. Validate on the next reject-with-instruction. The leftover (a) — narrow the #445 cooldown carve-out so a rejected TARGET exit also cools down — was **downgraded to an optional P2 stopgap** (low value now that the agent reads directives + the principal can edit levels directly).
...
Design doc: [`CONVICTION_EXPRESSION.md`](./plans/legacy/CONVICTION_EXPRESSION.md) (updated to v4).
