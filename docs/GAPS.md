...
- **`fieldChanges: {}` audit-diff bug (undercuts P1-33).** `update_thesis`'s diff builder drops changes — target/stop/trigger edits land with an empty `fieldChanges` (concrete case: XENE target 80→95 recorded as `{}` during PR-C validation). This directly falsifies P1-33's premise that "every step IS in the DB with exact `fieldChanges`": you can't render a "floor 64→71" timeline off an empty diff. A board chip exists but is unstarted. Fix: repair the diff builder so every field mutation records its from→to. **Prerequisite for P1-33's trigger-timeline slice.**
