...
**Remaining (the only open part):** a follow-up migration to drop the two Prisma models + tables — `AnalystBriefing` (641 rows) + `MorningBrief` (120 rows) and their relation fields on `AgentConfig`/`ResearchRun`. Deferred to "after #451 deploys clean" (the documented tables-drop-last ordering). Do NOT touch the intelligence core (`signal-*`, `read_signals`, `signal-router`, `Signal`/`AnalystSignalRoute`) or the podcast `PodcastSegmentBriefing` — separate + live. **Closes when the drop migration lands.**
...
- **Trailing stop as a first-class trigger predicate** (principal, 2026-06-23). Add a `TRAILING_STOP` kind so "exit X% from the peak" renders + edits in the trigger popover like any other trigger (`Exit if · trailing stop · 4%`), replacing the `Position.exitStrategy="TRAILING"` + `trailingStopPct` side-channel only the agent can set. Scope: ~6 exhaustive-switch sites (`format.ts`, `evaluate.ts`, `live-evaluate.ts`, `defaults.ts`, `needs-action.ts`, `intraday-tactical.ts`) + enforcement via the **price-monitor** (it already tracks `Position.peakPrice`; mirror `trailPct` onto `Position.exitStrategy` on write, never-fires in the generic evaluators) + a "change type" affordance on the stop trigger. ~half a day. Pairs with the trigger-value-edit popover shipped in #457 (`editableTriggerField` / `applyTriggerValueEdit`).
...
- **Portfolio Digest residual polish** (P1-23 core shipped — see `GAPS_HISTORY.md`): per-name `dayChangePct` is null in the digest; the homepage Snapshot/Activity/Theses layout reorg is a deferred UX call.
- **Dead `recentRuns` query in `getAnalystDetail`** — orphaned by #442 (unused after the RunCard extraction). Pure cleanup.
