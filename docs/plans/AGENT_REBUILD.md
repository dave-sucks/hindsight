...
>
> **Status:** plan. Section 1 is what's wrong, stage by stage, with the
> evidence. Section 2 is what each stage becomes. Section 3 is the trigger
> vocabulary the plan needs. Section 4 is the PR list. Section 5 is your
> part.
...
- `entry_kind`: `NOW` (condition true today and inside the chase limit) |
  ...
  the `ENTRY_AT_PRICE` flag are deleted.**
...
- **Fired buy = a decision.** Two answers: `place_trade`, or set the plan
  ...
  level moved above the price to avoid buying. `complete_run` refuses a
  fired ENTER resolved by a level raise with no cited structure.
...
- **Regime.** CAUTION halves new entries and blocks breakout setups;
  RISK_OFF allows event and mean-reversion entries only. Enforced in
  `place_trade` with the regime in the message.
...
### 2.7 Sizing → risk, heat, concentration

`position-sizing.ts` gains `riskPctForConviction`, `sharesForRisk`,
`openRisk` + a heat cap (6%), an industry cap (2 names) and a theme cap
(30–40% of heat), and `regime()`. Three analyst settings (risk band, heat
cap, regime policy) in the same Settings UI, same helper, so the number on
screen is the number that refuses the trade. Binary catalysts halve risk
and cap the −50%-gap loss at 0.5%.
...
Deleted: nothing from the schema (old rows must parse); the writer stops
authoring signal kinds; the RSI stub goes.

---

## 4. The PR list

Order: inputs → vocabulary → judgment → money. Each PR names its deletion.
Sizes in working days.

| # | PR | Adds | Deletes | Acceptance | Size |
|---|---|---|---|---|---|
| 1 | **The chart** — price-structure module, one-year bars, SPY/sector bars, data-block "Price structure", `get_stock_data.technicals` | 2.3 inputs, tests on synthetic bars | 90-day window; hand-rolled SMA/RSI in `get-stock-data.ts` | A MSFT writer run cites the 200-day, ATR, pivot and RS in dollars/percent | 3 |
| 2 | **Triggers that fire** — indicator snapshot + evaluator; close basis; `VOLUME_RATIO`, `NEW_HIGH`, `NEAR_SMA`, `PCT_FROM_52W_HIGH`, `RS_VS_SPY`, `GAP_UP`, 5D/20D moves; real RSI; VS_SMA fires | snapshot table + 06:30 job; popover support via existing dialog | RSI stub; VS_SMA carve-outs | GD/SYK entries evaluate; a test proves `AND[PRICE_ABOVE close, VOLUME_RATIO]` fires once on the close | 4 |
| 3 | **Setup catalog** — `setups.ts` (D1–D12 as data), `read_knowledge_library` serves it, `Thesis.setupId` | catalog + column | — | Catalog unit-tested for shape; no behaviour change yet | 1 |
| 4 | **The writer** — (rebases on #621's earnings block in the writer prompt; keeps it) setup-driven plan; `entry_kind` NOW/CONDITIONAL/UNPRICED; `entry_condition`; stop/target bases validated in-loop vs ATR; risk-based share suggestion; dispatch carries `setup_id` + screen row | writer prompt rewrite, schema fields, persist path | "never the current price" (prompt + schema + data block); `ENTRY_AT_PRICE`; hand-authored signal rungs; the horizon-default ENTER template as the only shape | PEAD writer on a fresh beat-and-raise returns NOW; a breakout thesis stores a composed entry with close+volume; DOCU/FIVE/HPE re-dispatched as the live proof | 5 |
| 5 | **The daily run** — (rebases on #621's "earnings on your book" section and its earnings-review branch; keeps both) fired-buy resolution law; book-health block; cash duty; regime; `ENTRY_CHASED` / `ENTRY_STALE`; setup-aware review | prompt + `get_portfolio_context` + plan-sanity | the "retune" paragraph as an equal answer; the per-horizon data-discipline block; the conviction→dollar table | A run with a fired ENTER either proposes or sets down; a level raise with no cited structure is refused by `complete_run` | 3 |
| 6 | **The tactical run** — per-setup confirmation; template re-ladder; same-bucket fire collapse | prompt + tactical-run | the horizon volume table | SRRK-shape double fire → one run; a breakout fire past the chase limit passes with the reason | 2 |
| 7 | **Sell rules per horizon** — per-horizon DEFAULT ladders; compounder wired; partial/8-week/time/beat-and-fade rungs; migration for the 8 held names (you approve) | horizon-keyed defaults + migration | `standingProtectionTriggers` as the single seed; the one-horizon `seedAccount` | ASML/CEG/WST carry 15/25 not 8; a TARGET trail is max(3 ATR, 12%); declined-sell count over the next two weeks | 3 |
| 8 | **Sizing** — `sharesForRisk`, heat cap, industry/theme caps, regime; three settings; `place_trade` gate with reasons | sizing functions + settings UI | the conviction dollar picker as the primary path | A STRONG buy with a 14% stop on a $100k book proposes ~$9k; a breakout in CAUTION is refused with the regime named | 3 |
| 9 | **Discovery by screens** — `screens.ts`, universe list, `run_screen`, prompt rewrite, computed composite defaults, Sunday cron on + 7:45 PEAD/gap run | screens + tool + prompt + cron | headline/mover/calendar pools in the prompt and #621's prose earnings-discovery step, which the PEAD screen replaces (`feeds`/`read_signals` already removed by #625/#621) | A Sunday run dispatches ≥ 1 writer from a screen row with numbers; the PEAD seat gets a Tuesday-morning gap candidate without you | 5 |
| 10 | **Insider + revisions** — daily Form-4 cluster job, `INSIDER_CLUSTER`; `ESTIMATE_REVISION` if the vendor serves it | jobs + predicates + screen inputs | — | A cluster on a book name fires a review with the buyers listed | 2 |
| 11 | **Scorecard by setup** — scorer grouping, /performance table, one-line-per-setup prompt input | | | /performance shows per-setup R and give-back | 2 |

≈ 33 working days. PRs 1–2 change nothing visible and unlock everything;
3–4 change what gets written; 5–6 change what gets bought; 7–8 change what
you get asked to sell and how big; 9–11 make the seats self-sourcing and
self-correcting. If you want the inbox quieter first: 1 → 2 → 7 → 8, then
3 → 4 → 5 → 6 → 9 → 10 → 11.

**Standing rules for every PR here.** No new write-time refusal without
the input that makes it rare. Every PR states its net line delta and names
its deletion. Prompts describe judgment; tools carry mechanics; the
catalog carries the rules. Cost stays flat: screens, indicators and the
catalog are arithmetic and data, not tokens; the writer prompt gains a
price-structure block and loses trigger boilerplate.

---

## 5. Your part

**Rulings (before the work):**
...
1. The catalog's numbers are the playbook's numbers (7–8% trade stop, 1.5×
   ...
   code.
2. Risk% band by conviction (E4) and the heat cap — defaults 0.5–1.25% and
   6%.
