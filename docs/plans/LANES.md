...
| **Owns** | Vendor clients and data modules in `lib/market-data/` except the chart; the earnings calendar (`earnings-calendar.ts`, `lib/agent/triggers/earnings.ts`); insider buying (`insider-cluster.ts`); SEC EDGAR; the pull tools `get_earnings_calendar`, `get_earnings_data`, `get_sec_filings`, `get_market_movers`; the Earnings page and stock-page data tabs; the vendor probe | The chart (`price-structure.ts`, `indicator-snapshot.ts`, `quote-age.ts`); the setup catalog (`lib/agent/knowledge/setups.ts`); every agent prompt (writer, daily run, tactical run, discovery, chat); `record_thesis`, `update_thesis`, `place_trade`, `manage_position`; trigger ops (`ops.ts`); plan-sanity flags; sizing; sell rules on analysts; the scorecard |
