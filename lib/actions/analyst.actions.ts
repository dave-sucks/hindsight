...
// ── resetAnalystIntelligence (danger action — wipe auto-state, re-normalize) ──
//
// The "clean slate" the Editor offers when a user wants to rebuild an
// analyst's intelligence surface:
//   1. Delete every BUILDER + BRIEFING_AGENT monitor on this analyst.
//      (User-origin monitors survive — the user explicitly owns those.)
//   2. Re-run the canonical normalizers against stored sectors / industries /
//      themes / exclusionList. Idempotent — if everything is already
//      canonical this step is a no-op.
//   3. Null out AnalystSignalRoute.status back to PENDING? NO — we leave
//      historical routes alone. Only the *future* routing pipeline gets
//      the benefit of the fresh monitor set + canonical values.
//
// Returns counts of what was touched so the UI can show what happened.

export interface ResetIntelligenceResult {
  monitorsDeleted: number;
  sectorsBefore: string[];
  sectorsAfter: string[];
  industriesBefore: string[];
  industriesAfter: string[];
  themesBefore: string[];
  themesAfter: string[];
  exclusionListBefore: string[];
  exclusionListAfter: string[];
}

export async function resetAnalystIntelligence(
  analystId: string,
): Promise<ResetIntelligenceResult> {
  const userId = await getCurrentUserId();
  if (!userId) throw new Error("Not authenticated");
  ...
  const sectorsBefore = (config.sectors ?? []) as string[];
  const industriesBefore = (config.industries ?? []) as string[];
  const themesBefore = (config.themes ?? []) as string[];
  const exclusionListBefore = (config.exclusionList ?? []) as string[];

  const sectorsAfter = normalizeSectors(sectorsBefore);
  const industriesAfter = normalizeIndustries(industriesBefore);
  const themesAfter = normalizeThemes(themesBefore);
  const exclusionListAfter = Array.from(
    new Set(exclusionListBefore.map((s) => s.toUpperCase())),
  );

  const [{ count: monitorsDeleted }] = await prisma.$transaction([
    ...
    prisma.agentConfig.update({
      where: { id: analystId, userId },
      data: {
        sectors: sectorsAfter,
        industries: industriesAfter,
        themes: themesAfter,
        exclusionList: exclusionListAfter,
      },
    }),
  ]);
