    ...
    for (const config of configs) {
      const result = await step.run(`research-${config.id}`, async () => {
        ...
        try {
          ...
          // V2: Briefing is now generated by complete_run tool directly.
          // Only generate as fallback if agent didn't call complete_run
          if (currentRun && currentRun.status === "RUNNING") {
            try {
              console.warn(`[morning-research] Run ${run.id} still RUNNING after agent finished. Generating fallback briefing.`);
              ...
              console.warn("[morning-research] Fallback briefing update failed (non-fatal):", briefingErr);
            }
          }
