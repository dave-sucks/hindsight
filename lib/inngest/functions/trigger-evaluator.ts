...
/**
 * Predicate kinds this cron can evaluate.
 *
 ...
 * `ctx.earnings`, which the cron fetches for the whole firm in one call.
 * Before that they waited on a routed signal that never carried a surprise
 * figure: 57 earnings triggers across 30 names, zero fires, ever. See
 * lib/agent/triggers/earnings.ts.
 */
    ...
    const cronFires = await step.run("evaluate-cron", async () => {
      ...
      for (const { thesis, analystId, ladder: triggers } of candidates) {
        ...
        for (const t of fires) {
          ...
          // REVIEW-batching: a REVIEW means "re-evaluate this thesis", not
          // "act now" — it converts to a trade ~4-8% of the time, so it
          // doesn't warrant its own tactical run. Write the TRIGGER_FIRED
          // audit row (stamped with the price that fired it) and skip the
          // spawn. The daily run picks it up via needsAction=TRIGGER_FIRED
          // and can still buy / sell / stop-watch / mark-reviewed from
          // there. That write is load-bearing: the daily run skips
          // unflagged theses, so without it a REVIEW that no longer matches
          // by morning would be silently dropped. ENTER/EXIT/stop triggers
          // still spawn a tactical. See OPENAI_COST_REDUCTION.md #2.
          if (action === "REVIEW") {
            await writeThesisUpdate({
              thesisId: thesis.id,
              type: "TRIGGER_FIRED",
