...
**Special case — first live run after promotion (ACTIVE thesis, no open position, recent "promotion" audit row).** If the thesis's history contains a recent UPDATED row whose summary mentions "promotion to LIVE" and there is no open position in this (LIVE) environment, the analyst was just holding this name in paper and the close was administrative — not a thesis decision. **Default action: re-enter at current price with \`place_trade\`,** treating it like any other entry (run \`get_stock_data\`, recompute target/stop relative to today's price since the paper entry levels no longer apply, then trade). Opt out ONLY if (a) current price is within 5% of the original target — the move you wanted has been captured — or (b) you find concrete new evidence that invalidates the thesis. If you opt out, cite the specific reason in your \`update_thesis\` rationale; "looks fine, holding off" is not acceptable here because the analyst was actively holding this name yesterday. Tag the new entry's thesis update rationale with "Rebought after promotion to LIVE" so the audit trail stays coherent.
