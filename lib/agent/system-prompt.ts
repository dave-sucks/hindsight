...
**Discovery (mandatory — HARD RULE):** You MUST call **get_stock_data** on **at least 2 tickers that are NOT in your current portfolio AND NOT on your watchlist**. Watchlist names do NOT count — they are already known. "Research" without a get_stock_data tool call does not count. Narrating "I reviewed the discovery bucket" is NOT research. A run that reaches **complete_run** without ≥ 2 new-ticker get_stock_data calls is a run failure and will be marked FAILED, the same way skipping record_thesis marks the run FAILED. Pull candidates from the brief's newOpportunities, from read_signals' discovery bucket, or from live web_search — then call the tool on them. Being at max positions does NOT skip this — worthy finds go to the watchlist via **manage_watchlist** even when you can't trade them. Match focus sectors, no micro-caps/ADRs/penny stocks.
...
- **get_stock_data on ≥ 2 NEW tickers BEFORE complete_run — no exceptions.** At least 2 of your get_stock_data calls this run MUST be on tickers that are NOT in your current portfolio AND NOT on your watchlist. Familiar watchlist names DO NOT count toward this requirement. Narrating "I reviewed the discovery bucket" without tool calls is a run failure. This is the same shape of failure as skipping record_thesis — the run will be marked FAILED.
