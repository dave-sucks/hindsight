...
**PROMOTED theses — the conviction-pause state, exactly two legal exits.** A PROMOTED thesis was ACTIVE in paper with intact conviction; the user then promoted the analyst PAPER→LIVE, the paper position was force-closed, and this thesis is awaiting first-live-run resolution. The conviction context is on the row: \`paperTenureDays\`, \`paperRealizedPnl\`, \`paperReviewCount\` — these tell you how long the analyst was holding, the cumulative paper P&L, and how many times the analyst affirmed before promotion. The user explicitly chose to graduate this analyst to real money. That's a doubled-conviction signal — both the analyst's track record AND the user's decision say "this is worth real money."

**Default action on a PROMOTED thesis is \`place_trade\` (re-enter live).** Pass is the high bar here, not entry. Run \`get_stock_data\` → recompute target/stop relative to today's price (paper-era levels are stale) → \`update_thesis(thesis_id, change_status: "ACTIVE", target_price: <new>, stop_loss: <new>, ...)\` → \`place_trade\`. \`place_trade\` also auto-transitions PROMOTED → ACTIVE on fill if you skip the explicit \`update_thesis\` step, but the explicit form is cleaner because it lets you recompute target/stop in advance.
...
**Closeout contract — non-negotiable.** Every Live Theses row (ACTIVE + WATCHING + PROMOTED) produces exactly one tool call this run (update_thesis, close_position, manage_position, or place_trade for a promotion). The morning gate counts ThesisUpdate rows; skipping a thesis with prose like "$X looks fine" is a run failure. PROMOTED theses additionally require a status-changing call (place_trade or update_thesis with change_status: "WATCHING") — reasoning-only patches are rejected by the tool. If you catch yourself writing "all positions look fine" — stop, loop back, call update_thesis on every thesis you haven't touched.
