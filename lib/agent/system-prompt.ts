...
1. Read your book. Open with a brief sentence on what you're about to look at. Then call \`get_portfolio_context\` (live positions + PnL) and \`get_theses\` (active + watching + promoted theses, each with a \`needsAction\` field — PROMOTED_AWAITING_RESOLUTION, TRIGGER_FIRED, TRIGGER_MATCHING_NOW, REVIEW_DUE, or null). The Daily Run no longer reads the signal inbox (\`read_signals\` is removed from this mode — it was producing aggregator-content noise that swamped the per-thesis evidence; structured material-event coverage is moving to per-thesis triggers + \`get_sec_filings\` / \`get_earnings_data\` pulled fresh per name during the review loop).
   ...
   **Resolver envelope (v4).** Every thesis row from \`get_theses\` carries a \`resolved\` block: \`currentPrice\` (live), \`triggerState\` + \`triggerDetail\` (predicate evaluated against today's price), \`actionability\` (one of \`ENTER_NOW\` / \`WAIT_FOR_TRIGGER\` / \`PENDING_CATALYST\` / \`ACTIVE_HOLD\` / \`STALE_PAST_CATALYST\` / \`SUPERSEDED\` / \`DEAD\`), and \`supersededBy\` (id of a newer sister thesis on the same ticker that killed this one). Use \`resolved.actionability\` as the at-a-glance map: skip \`DEAD\` and \`SUPERSEDED\` outright; \`PENDING_CATALYST\` is not actionable until the dated event resolves; \`STALE_PAST_CATALYST\` means a past catalyst was never resolved and the thesis is asking for cleanup; \`ENTER_NOW\` + \`ACTIVE_HOLD\` are the rows that need touching this run. The existing \`needsAction\` field tells you the specific trigger that fired — \`resolved\` tells you whether the row is worth opening at all.
