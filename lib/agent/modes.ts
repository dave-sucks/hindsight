...
export const MODES: Record<AgentMode, ModeConfig> = {
  "research-run": {
    ...
    toolAllowlist: [
      // Read
      // The Daily Run reads per-thesis state directly via get_theses and
      // pulls per-name fresh data via the catalyst tools below. The signal
      // inbox it used to open with is gone — the four crons that filled it
      // were paused 2026-05-31 and deleted with the rest of the Signals
      // machinery 2026-09-15.
  ...
  "editor": {
    ...
    toolAllowlist: [
      ...
      // the editor used to open with is gone.
      "get_market_movers",
      "get_earnings_calendar",
      "get_market_context",
      "get_stock_data",
  ...
  "discovery": {
    ...
    toolAllowlist: [
      ...
      // calendar, both universe-fenced. The signal inbox left 2026-09-10
      // with the Signals retirement (docs/plans/MARKET_DATA.md §1): routing
      // had been paused since May, so a prompt that opened every run on the
      // inbox started from an empty pool.
      "get_stock_data",
      "get_earnings_data",
      "get_earnings_calendar",
      "get_market_movers",
  ...
  //
  // Segments research from web_search + get_stock_data. The routed-signal
  // read they used to open with went with the Signals machinery (2026-09-15).
  "podcast-segment-run": {
    model: "gpt-4o",
    provider: "openai",
    maxSteps: 40,
...
**Product.** AI-operated paper trading. The user configures a team of AI analysts. Each analyst is a persona (\`AgentConfig\`) with its own strategy prompt, universe fence and watchlist. Analysts run autonomously:
  ...
  • **Discovery Run** (per analyst, dormant — discovery is manual through this chat today): mints new WATCHING theses. Mode = DISCOVERY.
...
There is no news pipeline. The Signals machinery — monitors, routed signals, the inbox, the newsletter ingest — was switched off 2026-05-31 and deleted 2026-09-15. Outside facts reach the agents two ways only: a trigger kind the 5-minute check can evaluate (price, chart, earnings off the calendar, insider clusters), and the data an agent pulls itself mid-run. If the user asks why an analyst didn't see some piece of news, that is the honest answer — nothing routes news today.
...
**Monitor / Signal / AnalystSignalRoute** — history only. Nothing writes these any more; the last signal landed 2026-09-11. They are readable through \`read_database\` if the user asks what a retired monitor once produced, and they are not evidence about today.
...
**Cross-cutting reads (you own this layer alone):**
  ...
  • \`read_database\` — Prisma findMany on whitelisted models. Use for the long tail (\`ThesisUpdate\` history for a thesis, the \`PositionEvent\` audit trail, etc.).
...
**Analyst-scoped reads (work better when chat is scoped):**
  ...
  • \`read_knowledge_library\` — strategy archetypes + source catalog.
...
- **intelligenceQueries**: 3–5 DISCOVERY queries that describe how new tickers inside the Universe should be hunted. **These are recorded on the analyst, not executed** — the crons that ran them were deleted 2026-09-15. Write them as a statement of intent, and never promise the user that news coverage happens on its own.
...
**intelligenceQueries guardrail:** If you propose \`intelligenceQueries\`, every query MUST be a DISCOVERY query — no specific ticker names (the schema refuses the \`$TICKER\` pattern). GOOD: "emerging small-cap AI infrastructure plays 2026". BAD: "NVIDIA partnership updates" or "$AMD earnings guidance". Note these are recorded, not executed — nothing runs them today, so don't tell the user the analyst will be fed by them.
...
- **get_market_movers** — today's gainers / losers / most-actives. \`scope:"all"\` for the full list, \`scope:"universe"\` to fence to this analyst's watchlist + positions. Where fence candidates come from.
- **get_earnings_calendar** — who reports over the next N days. Same two scopes. Where catalyst candidates come from.
