...
describe("computeNeedsAction — HELD_THROUGH_FLOOR (P1-39, PROPOSAL_FATIGUE.md)", () => {
  // MU shape: held LONG with a protective floor the price has breached; the
  // principal rejected / let expire the protective exit proposals but the
  // system kept re-proposing the same line — 49× over 22 days. The flag
  // replaces "re-propose the exit" with "move the line."
  const floorExit: Trigger = {
    id: "trig-floor",
    predicate: { kind: "PRICE_BELOW", level: 86 },
    action: "EXIT",
  ...
  const heldThrough = {
    heldThroughCount: 3,
    rejectMessage: "hold, but re-propose if it drops more",
    recentLow: 83.1,
  };
  const heldThesis = {
    ...baseThesis,
  ...
  it("flags a HOLDING with recent protective declines and price still under the floor", () => {
    const result = computeNeedsAction({
      thesis: heldThesis,
      latestUpdate: null,
      latestQuote: { price: 84.5, changePct: 0 }, // under the $86 floor
      now,
      heldThroughFloor: heldThrough,
    });
    expect(result).toEqual({
      kind: "HELD_THROUGH_FLOOR",
      floorPrice: 86,
      floorSummary: "price < $86.00",
  ...
  it("outranks TRIGGER_FIRED on the same floor rung (the fire IS the fatigue loop)", () => {
    const result = computeNeedsAction({
      thesis: heldThesis,
      latestUpdate: {
        type: "TRIGGER_FIRED",
        triggerId: "trig-floor",
        timestamp: new Date("2026-05-10T09:00:00Z"),
      },
      latestQuote: { price: 84.5, changePct: 0 },
      now,
      heldThroughFloor: heldThrough,
    });
    expect(result?.kind).toBe("HELD_THROUGH_FLOOR");
  });

  it("does NOT flag when price recovered above the floor (the line held)", () => {
    const result = computeNeedsAction({
      thesis: heldThesis,
      latestUpdate: null,
      latestQuote: { price: 88, changePct: 0 }, // back above $86
      now,
      heldThroughFloor: heldThrough,
    });
    // Falls through to the lower-precedence checks; with no fire/match/review
    // pending, the row needs nothing — a future re-breach is a fresh alert.
    expect(result).toBeNull();
  });

  it("does NOT flag without held-through context (no recent declines)", () => {
    const result = computeNeedsAction({
      thesis: heldThesis,
      latestUpdate: null,
      latestQuote: { price: 84.5, changePct: 0 },
    ...
    // No heldThroughFloor input → the breached floor surfaces through the
    // normal trigger paths instead (here: TRIGGER_MATCHING_NOW on the EXIT).
    expect(result?.kind).toBe("TRIGGER_MATCHING_NOW");
  });

  it("does NOT flag when the ladder has no protective floor rung", () => {
    const result = computeNeedsAction({
      thesis: { ...heldThesis, triggers: [] },
      latestUpdate: null,
      latestQuote: { price: 84.5, changePct: 0 },
      now,
      heldThroughFloor: heldThrough,
    });
    // No floor → nothing to trail; the position is DOWN vs entry so neither
    // P&L flag fires and there's no stale line to move.
    expect(result).toBeNull();
  });

  it("does NOT flag a non-HOLDING row (the matching trigger surfaces instead)", () => {
    const result = computeNeedsAction({
      thesis: { ...heldThesis, status: "WATCHING" },
      latestUpdate: null,
      latestQuote: { price: 84.5, changePct: 0 },
      now,
      heldThroughFloor: heldThrough,
    });
    // Held-through is a HOLDING-only concept; on WATCHING the matching EXIT
    // predicate surfaces through the normal trigger path.
    expect(result?.kind).toBe("TRIGGER_MATCHING_NOW");
  });

  it("PROMOTED still outranks HELD_THROUGH_FLOOR", () => {
    const result = computeNeedsAction({
      thesis: { ...heldThesis, status: "PROMOTED" },
      latestUpdate: null,
      latestQuote: { price: 84.5, changePct: 0 },
      now,
      heldThroughFloor: heldThrough,
    });
    expect(result?.kind).toBe("PROMOTED_AWAITING_RESOLUTION");
  });

  it("SHORT: flags when price is still ABOVE a protective ceiling, recentLow carries the recent high", () => {
    const shortFloor: Trigger = {
      id: "trig-short-floor",
      predicate: { kind: "PRICE_ABOVE", level: 50 },
      action: "EXIT",
      rationale: "short stop",
      cooldownDays: 0,
    };
    const result = computeNeedsAction({
      thesis: {
      ...
      latestUpdate: null,
      latestQuote: { price: 52, changePct: 0 }, // above the $50 ceiling
      now,
      heldThroughFloor: { ...heldThrough, recentLow: 53.2 },
    });
    expect(result?.kind).toBe("HELD_THROUGH_FLOOR");
    if (result?.kind === "HELD_THROUGH_FLOOR") {
      expect(result.floorPrice).toBe(50);
      expect(result.recentLow).toBe(53.2);
    }
  });

  it("degrades gracefully: recentLow null rides through, flag still fires", () => {
    const result = computeNeedsAction({
      thesis: heldThesis,
      latestUpdate: null,
      latestQuote: { price: 84.5, changePct: 0 },
      now,
      heldThroughFloor: { ...heldThrough, recentLow: null, rejectMessage: null },
    });
    expect(result?.kind).toBe("HELD_THROUGH_FLOOR");
    if (result?.kind === "HELD_THROUGH_FLOOR") {
      expect(result.recentLow).toBeNull();
