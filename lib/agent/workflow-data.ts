...
// ── Analyst overview (for /analysts/[id] page) ─────────────────────────────

export const ANALYST_INTELLIGENCE_DETAILS: DetailSection[] = [
  {
    heading: "How this analyst gets intelligence",
    items: [
      { label: "Search monitors", value: "Custom queries defined during analyst creation, run every morning at 6:30 AM via Perplexity Sonar. Example: 'semiconductor supply chain news'. The briefing agent also creates dynamic monitors with expiration dates for time-sensitive topics." },
      { label: "Domain monitors", value: "Tracked websites (e.g., Electrek, STAT News, Reuters) checked daily via domain-filtered Sonar search. High-priority sources get full article extraction via Firecrawl for deeper reading." },
      { label: "Ticker monitors", value: "Every open position and watchlist item is searched automatically at 7:00 AM. Tickers are deduplicated across analysts — NVDA is searched once even if 3 analysts hold it." },
      { label: "Signal routing", value: "At 7:30 AM, the router scores every finding against this analyst: +40 for ticker match, +20 for sector overlap, +15 for theme/keyword match, +15/+10 for BREAKING/HIGH urgency. Findings scoring 15+ are routed here." },
      { label: "Intelligence policy", value: "This analyst's policy controls how many signals per run, how many articles to read, attention weights (how much time on holdings vs watchlist vs discovery), live search budget, and quality floors. Set during creation, adjustable anytime." },
    ],
  },
  {
    heading: "How the agent reads intelligence",
    items: [
      { label: "Phase 1: read_morning_brief", value: "The agent's first tool call. Reads the pre-generated morning brief with market context, portfolio alerts, watchlist updates, new opportunities, and risk flags. If the brief covered market conditions, the agent skips get_market_context entirely." },
      { label: "Phase 1: read_signals", value: "Reads the individual findings routed to this analyst. Filterable by tickers, themes, signal type, and urgency. The agent uses these to decide what to research deeper." },
      { label: "Phase 1: read_artifact", value: "When a finding's headline is interesting, the agent reads the full extracted article (up to 4,000 chars). Stored as Artifacts by the Firecrawl extraction step." },
    ],
  },
];

export const ANALYST_BRIEF_DETAILS: DetailSection[] = [
  {
    heading: "Morning intelligence briefs (pre-run)",
    items: [
      { label: "When", value: "7:45 AM ET every weekday, after signal routing completes. Generated by GPT-4o reading this analyst's routed findings, current positions, and watchlist." },
      { label: "What's in it", value: "Market context (3-4 sentences on regime and themes), portfolio alerts (per-position flags like 'near target' or 'earnings this week'), watchlist updates (news on watched tickers), new opportunities (max 3, matched to mandate), attention priority (top 5 tickers), and risk flags (concrete, time-bound warnings)." },
      { label: "How the agent uses it", value: "The brief is the agent's first read in every session. Portfolio alerts prioritize which holdings to review. New opportunities seed the discovery phase. Risk flags inform position sizing and exit decisions." },
    ],
  },
  {
    heading: "Post-run standup briefs (memory)",
    items: [
      { label: "When", value: "Immediately after every research session — both manual runs and daily crons. A separate briefing agent (GPT-4o) reviews the full session." },
      { label: "What the reviewer reads", value: "The full conversation transcript (every message, tool call, and result), portfolio state with unrealized P&L, all theses and trades from this session, and the previous standup brief for continuity." },
      { label: "What it produces", value: "Narrative (400-600 words of what actually happened), strategy notes (data-driven assessment), market posture (e.g. 'cautiously bullish'), watch-tomorrow items (specific tickers with triggers), unresolved items, self-corrections (behavioral patterns to fix), and 0-5 dynamic monitors with expiration dates." },
      { label: "Why separate agents?", value: "The research agent (GPT-4.1) does research and trading. A separate reviewer (GPT-4o) writes the standup. You don't ask the trader to write their own performance review — the reviewer reads the full transcript and produces an honest assessment with no ego." },
      { label: "Run-to-run continuity", value: "The standup feeds into the next session's system prompt. The agent MUST reference watch-tomorrow items in Phase 0. Dynamic monitors are picked up by the next morning's intelligence sweep automatically." },
    ],
  },
];

export const ANALYST_BUILDER_DETAILS: DetailSection[] = [
  {
    heading: "Creating an analyst",
    items: [
      { label: "The conversation", value: "You describe your trading style — sectors, patterns, risk appetite. The AI builder (GPT-4o) pushes you to think deeper, then researches live market data using 4 tools (market context, stock data, earnings, SEC filings) before proposing anything." },
      { label: "What it creates", value: "A complete analyst config: strategy document (3-5 paragraph playbook), trading parameters (direction bias, hold durations, confidence threshold, position sizing), watchlist, domain monitors, search monitors, and intelligence policy." },
      { label: "Review & iterate", value: "Everything appears in a side panel. You can ask for changes to any field through conversation. The builder validates suggestions against live data before applying." },
    ],
  },
  {
    heading: "Editing an analyst",
    items: [
      { label: "Same experience", value: "The editor uses the same AI chat + config panel pattern as the builder. Same GPT-4o model, same 4 research tools for validation." },
      { label: "What you can change", value: "Strategy prompt, direction bias, hold durations, sectors, confidence threshold, position sizing, watchlist, exclusion list, monitors, and intelligence policy. Changes take effect on the next run." },
      { label: "Direct config editing", value: "You can also edit trading parameters directly via the settings sheet (gear icon) without going through the AI chat — for quick adjustments to numbers and toggles." },
    ],
  },
];

// ── Trade lifecycle (for /trades page) ──────────────────────────────────────

export const TRADE_LIFECYCLE_DETAILS: DetailSection[] = [
  {
    heading: "How trades happen",
    items: [
      { label: "Thesis first", value: "Before any trade, the analyst must record a thesis — direction (LONG/SHORT), confidence score, entry/target/stop prices, reasoning, and risk flags. The thesis ID is required to place a trade." },
      { label: "Confidence gate", value: "Each analyst has a minimum confidence threshold (e.g. 70%). The analyst must meet this bar in its thesis before trading. Below the threshold, it records a PASS or adds to the watchlist instead." },
      { label: "Execution", value: "The place_trade tool submits a market order on Alpaca's paper trading platform. It waits up to 5 seconds for a fill, records the entry price, creates a Position, and logs a TradeDecision. If the stock was on the watchlist, it automatically graduates off." },
      { label: "Duplicate protection", value: "An analyst cannot open a second position in a stock it already holds. The system blocks the trade and logs it as FAILED." },
    ],
  },
  {
    heading: "Position tracking",
    items: [
      { label: "Live P&L", value: "Every open position tracks unrealized P&L from real market prices via Alpaca. The analyst sees current P&L, days held, and proximity to target/stop at the start of each run." },
      { label: "Price monitoring", value: "An hourly cron checks all open positions. When a position reaches 80%+ of its target or stop-loss, it's flagged as NEAR_TARGET or NEAR_STOP, and the next run prioritizes reviewing it." },
      { label: "Exit decisions", value: "The analyst can close positions via close_position with a reason: TARGET (thesis played out), STOP (thesis failed), or MANUAL (rebalancing). Exits happen before entries in Phase 6 to free capital." },
    ],
  },
  {
    heading: "Post-trade evaluation",
    items: [
      { label: "Trade evaluator", value: "When a position closes, a GPT-4o evaluator reviews the trade: was the thesis correct? Was timing good? What lesson should the analyst learn? The evaluation is stored on the Position and feeds into future sessions." },
      { label: "Pass tracking", value: "Stocks the analyst passes on are recorded with the price at that moment. Later, the system checks if the stock moved — measuring whether the analyst is too conservative or too aggressive with passes." },
      { label: "Weekly accuracy", value: "Every Sunday, an accuracy scorer calculates win rate, calibration (does 80% confidence actually win 80% of the time?), and per-sector performance. The analyst sees these stats at the start of every run." },
    ],
  },
];

// ── Performance details (for /performance page) ─────────────────────────────

export const PERFORMANCE_DETAILS: DetailSection[] = [
  {
    heading: "How performance is measured",
    items: [
      { label: "Win rate", value: "Percentage of closed trades that were profitable. Calculated per analyst and overall. A trade is a WIN if realized P&L > 0, LOSS if < 0." },
      { label: "Calibration", value: "Does the analyst's confidence predict actual outcomes? If it says 80% confidence but only wins 50% of the time, it's overconfident. The accuracy scorer flags this and the agent sees a calibration note in its system prompt." },
      { label: "Sector breakdown", value: "Win rate and average P&L per sector. Shows which sectors the analyst is strong in and which it should avoid or improve on." },
    ],
  },
  {
    heading: "When it updates",
    items: [
      { label: "Real-time P&L", value: "Open position P&L updates from live Alpaca prices. Closed trade outcomes are final at close time." },
      { label: "Weekly accuracy report", value: "Every Sunday at 10 AM ET, the accuracy scorer runs. It calculates per-analyst win rate, total trades analyzed, and writes a calibration note. The analyst reads this at the start of every Monday session." },
      { label: "End-of-day snapshots", value: "Every trading day, an EOD job snapshots all open positions with closing prices. This builds the equity curve chart you see on the dashboard and analyst detail pages." },
    ],
  },
];

// ── Dashboard details (for / page) ──────────────────────────────────────────

export const DASHBOARD_DETAILS: DetailSection[] = [
  {
    heading: "What you see on the dashboard",
    items: [
      { label: "Equity curve", value: "Portfolio value over time, built from end-of-day snapshots of all open positions plus realized P&L from closed trades. Starting value is the $100k paper account." },
      { label: "Recent picks", value: "The latest theses from all analysts — LONG, SHORT, and PASS verdicts with confidence scores and price targets. Filterable by direction." },
      { label: "Open positions", value: "All currently held paper trades across all analysts. Shows live unrealized P&L, entry price, and the analyst that placed the trade." },
      { label: "Closed trades", value: "Recently closed positions with realized P&L, outcome (WIN/LOSS), and close reason (TARGET/STOP/MANUAL)." },
    ],
  },
  {
    heading: "The daily cycle",
    items: [
      { label: "6:30–7:45 AM", value: "Intelligence pipeline gathers market data, routes findings, and generates morning briefs for each analyst." },
      { label: "8:00 AM", value: "Each enabled analyst runs an automated research session — reads intelligence, reviews holdings, discovers opportunities, executes trades." },
      { label: "After each run", value: "A separate briefing agent writes a standup memo. Dynamic monitors are created for next morning's sweep. The loop continues daily." },
      { label: "Hourly", value: "Price monitor checks all open positions for proximity to targets and stop-losses." },
      { label: "End of day", value: "EOD snapshot captures closing prices for the equity curve." },
      { label: "Weekly", value: "Sunday accuracy scorer calculates win rate and calibration per analyst." },
    ],
  },
];
