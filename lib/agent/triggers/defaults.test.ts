...
/**
 * Entry-trigger SIDE follows the level the analyst chose (2026-08-27).
 *
 * Before this, every LONG watch got PRICE_ABOVE(entryPrice) regardless of
 * intent, which broke both shapes at once on the live book:
 *   • a pullback level was true the moment it was written — NOW set $130
 *     against a $132.51 tape, ABT $112 against $116.44 — so the rung fired
 *     every cooldown and the analyst correctly declined each time.
 *   • a confirmation level waited, which is right, but it was the only
 *     shape available, so dip-buying seats got breakout semantics by
 *     accident.
 *
 * Numbers below are the real ones from ThesisUpdate.fieldChanges.
 */
describe("defaultTriggersForHorizon — ENTER side follows the chosen level", () => {
  const enterOf = (
    triggers: {
      action: string;
      rationale?: string;
      predicate: { kind: string; level?: number };
    }[],
  ) => triggers.find((t) => t.action === "ENTER");

  it("a LONG level set UNDER the tape is a pullback — fires when price comes back down", () => {
    // NOW, 2026-08-27: tape $132.51, analyst set $130.
    const t = defaultTriggersForHorizon(
      "TARGET",
      { entryPrice: 130, targetPrice: 175, stopLoss: 78, direction: "LONG", currentPrice: 132.51 },
      "WATCHING",
    );
    const enter = enterOf(t)!;
    expect(enter.predicate).toEqual({ kind: "PRICE_BELOW", level: 130 });
  });

  it("a LONG level set OVER the tape is a breakout — unchanged behaviour", () => {
    // CSCO, 2026-08-24: tape $111.04, analyst set $116.10.
    const t = defaultTriggersForHorizon(
      "TARGET",
      { entryPrice: 116.1, targetPrice: 130, stopLoss: 101.7, direction: "LONG", currentPrice: 111.04 },
      "WATCHING",
    );
    expect(enterOf(t)!.predicate).toEqual({ kind: "PRICE_ABOVE", level: 116.1 });
  });

  it("a level set AT the tape means buy it here — proposes immediately", () => {
    const t = defaultTriggersForHorizon(
      "CATALYST",
      { entryPrice: 14.33, targetPrice: 26, stopLoss: 11.5, direction: "LONG", currentPrice: 14.33 },
      "WATCHING",
    );
    expect(enterOf(t)!.predicate).toEqual({ kind: "PRICE_ABOVE", level: 14.33 });
  });

  it("no quote in context → keeps the historical breakout shape, no regression", () => {
    const t = defaultTriggersForHorizon(
      "CATALYST",
      { entryPrice: 65, targetPrice: 90, stopLoss: 43, direction: "LONG" },
      "WATCHING",
    );
    expect(enterOf(t)!.predicate).toEqual({ kind: "PRICE_ABOVE", level: 65 });
  });

  it("SHORT mirrors — a level set OVER the tape is a rally to sell into", () => {
    const t = defaultTriggersForHorizon(
      "TARGET",
      { entryPrice: 120, targetPrice: 90, stopLoss: 132, direction: "SHORT", currentPrice: 110 },
      "WATCHING",
    );
    expect(enterOf(t)!.predicate).toEqual({ kind: "PRICE_ABOVE", level: 120 });
  });

  it("SHORT under the tape stays a breakdown", () => {
    const t = defaultTriggersForHorizon(
      "TARGET",
      { entryPrice: 100, targetPrice: 80, stopLoss: 112, direction: "SHORT", currentPrice: 110 },
      "WATCHING",
    );
    expect(enterOf(t)!.predicate).toEqual({ kind: "PRICE_BELOW", level: 100 });
  });

  it("the rationale says which shape it is, so the thesis stays self-describing", () => {
    const dip = defaultTriggersForHorizon(
      "TARGET",
      { entryPrice: 112, targetPrice: 140, stopLoss: 84, direction: "LONG", currentPrice: 116.44 },
      "WATCHING",
    );
    expect(enterOf(dip)!.rationale).toContain("came back to");
    const brk = defaultTriggersForHorizon(
      "TARGET",
      { entryPrice: 190, targetPrice: 240, stopLoss: 110, direction: "LONG", currentPrice: 184.16 },
      "WATCHING",
    );
    expect(enterOf(brk)!.rationale).toContain("broke above");
  });
});
