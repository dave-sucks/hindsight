 ...
 * ACTIVE + WATCHING thesis. They were auto-attached by the pre-C1/C2
 ...
 * 2026-05-18 with no actionable output. C1/C2 in
 * docs/plans/SYSTEM_AUDIT_2026_05_19.md strips them from defaults; this
 * script repairs existing rows.
 *
      ...
      if (!DRY_RUN) {
        ...
        await prisma.thesisUpdate.create({
          data: {
            thesisId: t.id,
            type: "UPDATED",
            summary: `Removed ${removed} REVIEW_DATE_HIT trigger${removed > 1 ? "s" : ""} (C1/C2 cleanup)`,
            rationale:
              "Cleanup post C1/C2: daily-run's needsAction.REVIEW_DUE reads " +
