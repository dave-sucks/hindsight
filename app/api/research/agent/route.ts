  ...
  try {
    ...
    const result = streamText({
      ...
      async onFinish({ response, finishReason, usage }) {
        ...
        // Fallback: generate briefing if one doesn't exist yet — covers:
        // 1. Agent didn't call complete_run (hit step limit, RUNNING)
        // 2. complete_run ran but updateAnalystBriefing silently failed inside it
        // 3. Stream error left run in FAILED state
        ...
        if (briefingAnalystId) {
          // Check if a briefing already exists for this run (complete_run may have created it)
          ...
          if (!existingBriefing) {
            console.warn(`[agent] No briefing found for run ${runId} (status=${currentRun?.status}) — generating fallback briefing.`);
            waitUntil(
              updateAnalystBriefing({
                ...
                console.error("[agent] Fallback briefing generation failed:", err)
              )
            );
          } else {
            console.log(`[agent] Briefing already exists for run ${runId}, skipping fallback.`);
          }
        }
      },
    });
