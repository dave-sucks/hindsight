...
export const { GET, POST, PUT } = serve({
  ...
  functions: [
    ...
    // Restored 2026-05-11 after PR #238 deleted it — read on the Health page.
    syncHeartbeat,
    evaluateTrade,
    morningResearch,
    eodEvaluation,
    ...
    indicatorSnapshot,
    // Runs every 5 min during US market hours (plus the close pass). Emits
    // `app/thesis.trigger.fired` on match.
    triggerEvaluator,
    // PR 2 — consumes `app/thesis.trigger.fired`. Spawns a focused agent
    // run scoped to one (thesis, trigger) pair. The actual
    ...
    // agent run per analyst that looks for net-new ticker coverage and
    // mints WATCHING theses. Does NOT touch existing theses (the daily
    // run handles those). Discovery is manual by design today — the cron
    // is dormant.
    discoveryRun,
