...
/**
 * PodcastFindingsTab — segment-routed signals aggregated across the
...
interface PodcastFindingsTabProps {
  podcastId: string;
  /** Optional ticker suggestions for the filter combobox — pass any
   *  ticker the podcast surfaces frequently if available. */
  tickerSuggestions?: string[];
}

export function PodcastFindingsTab({
  podcastId,
  tickerSuggestions = [],
}: PodcastFindingsTabProps) {
  ...
  useEffect(() => {
    setLoading(true);
    const qs = new URLSearchParams({ podcastId, limit: "100" });
