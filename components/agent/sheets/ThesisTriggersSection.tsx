...
const COUNT_FROM_OPTIONS: ReadonlyArray<{ v: CountFrom; l: string }> = [
  ...
  { v: "BUY", l: "After buy" },
  { v: "EVENT_AFTER", l: "After event date" },
  { v: "EVENT_BEFORE", l: "Before event date" },
];
...
function countFromHelp(from: CountFrom, action: string, val: string, held: boolean): string {
  ...
  switch (from) {
    ...
    case "EVENT_AFTER":
    case "EVENT_BEFORE":
      return `${n} days ${from === "EVENT_BEFORE" ? "before" : "after"} the event date on the thesis (the FDA decision, the deal close): ${verb}. No event date on the thesis means it never fires.`;
    default:
